...
func TestToToolText(t *testing.T) {
	tests := []struct {
		...
		{name: "merge 405",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 405, Detail: "405 Method Not Allowed"}),
			prefix:   "405: слияние отклонено",
			contains: []string{"get_merge_request"}},
		{name: "merge 406",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 406, Detail: "406 Not Acceptable"}),
			prefix:   "406: слияние отклонено",
			contains: []string{"get_merge_request"}},
		{name: "merge 422 branch cannot be merged",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 422, Detail: "[Branch cannot be merged]"}),
			prefix:   "422: GitLab не смог влить MR",
			contains: []string{"get_merge_request"}},
		{name: "merge 409 source changed",
			err:    withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 409, Detail: "SHA does not match HEAD of source branch"}),
			prefix: "409: ветка-источник изменилась"},
		{name: "merge 400 sha required",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 400, Detail: "SHA must be provided when merging"}),
			prefix:   "400: ",
			contains: []string{"sha", "MRX-03"}},
		{name: "merge 401 is a permission problem, not a bad token",
			err:         withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindUnauthorized, Status: 401}),
			prefix:      "401: нет прав вливать этот MR",
			notContains: []string{"токен недействителен", "GitLab отклонил токен"}},
		{name: "merge 403 names the protected branch",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindForbidden, Status: 403}),
			prefix:   "403: ",
			contains: []string{"целевая ветка защищена", "роль"}},
		{name: "commit 403 keeps the Phase 2 wording",
			err:      withWrite(opCommit, "проект или ветка", &glclient.Error{Kind: glclient.KindForbidden, Status: 403}),
			contains: []string{"403: запись отклонена"}},
		{name: "405 on another op is generic",
			err:         withWrite(opCreateMR, mrSubject, &glclient.Error{Kind: glclient.KindBadRequest, Status: 405, Detail: "nope"}),
			exact:       "405: GitLab отклонил запрос: nope",
			notContains: []string{"слияние отклонено"}},
		{name: "merge 502 outcome unknown",
			err:      withWrite(opMergeMR, mrSubject, &glclient.Error{Kind: glclient.KindServer, Status: 502}),
			contains: []string{"Результат записи неизвестен", "get_merge_request"}},
