...
- Human-action перед первым прогоном (проект и оба PAT — `api` и `read_api` — автор уже создал): выставить в окружении `GITLAB_TOKEN` и `GITLAB_TOKEN_READONLY`; убедиться, что в sandbox в `main` лежит `README.md` (скрипт при первом шаге проверяет это и падает с понятным сообщением). Токены были вставлены в чат сессии обсуждения — после завершения Phase 4 их стоит отозвать и выпустить новые.
